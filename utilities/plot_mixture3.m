function plot_mixture3(training_data,class_id,color)
...
if(nargin < 3)
    color = {'ko', 'ro', 'go', 'bo', 'mo', 'co','k+','r+','g+','b+'};
end
clf
...
for(i = 1:length(ucids))
    if(ucids(i)<11)
        scatter3(training_data(1,find(class_id==ucids(i))),...
            training_data(2,find(class_id==ucids(i))),training_data(3,find(class_id==ucids(i))),color{ucids(i)})
        view([-13,10]);
    else
            scatter3(training_data(1,find(class_id==ucids(i))),...
                training_data(2,find(class_id==ucids(i))),training_data(2,find(class_id==ucids(i))),'k.')
    end
end
hold off
